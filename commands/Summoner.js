...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const link = regions.get(region.toUpperCase());

        // Summoner Fetch
        try {
            const summonerResponse = await axios.get(link + `lol/summoner/v4/summoners/by-name/${inputName}?api_key=${RIOTAPIKEY}`);
        ...
        // League Fetch
        try {
            const leagueResponse = await axios.get(link + `league/v4/entries/by-summoner/${summoner.id}?api_key=${RIOTAPIKEY}`);
        ...
        // Match Fetch
        try {
            const matchResponse = await axios.get(link + `lol/spectator/v4/active-games/by-summoner/${summoner.id}?api_key=${RIOTAPIKEY}`);
